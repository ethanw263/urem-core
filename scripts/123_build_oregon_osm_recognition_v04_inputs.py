#!/usr/bin/env python3
"""
123_build_oregon_osm_recognition_v04_inputs.py

Memory-safe streaming OSM extractor for Oregon Recognition v04 inputs.

Input:
- data/raw/osm/oregon-260615.osm.pbf

Outputs:
- data/processed/oregon_osm_recognition_features_v04.gpkg
- data/processed/oregon_osm_recognition_features_v04.csv
"""
...
SCRIPT_NAME = "123_build_oregon_osm_recognition_v04_inputs"

BASE_DIR = Path(__file__).resolve().parents[1]

OSM_PBF_PATH = BASE_DIR / "data/raw/osm/oregon-260615.osm.pbf"

OUT_GPKG = BASE_DIR / "data/processed/oregon_osm_recognition_features_v04.gpkg"
OUT_CSV = BASE_DIR / "data/processed/oregon_osm_recognition_features_v04.csv"

OREGON_CRS = "EPSG:5070"
...
def classify_feature(tags):
    ...
    if tourism == "information" or information in {
        "map",
        "board",
        "guidepost",
        "office",
...
def main():
    log("Starting Oregon OSM Recognition v04 extraction")
    ...
    gdf_projected = gdf.to_crs(OREGON_CRS)
    gdf["feature_area_m2"] = gdf_projected.geometry.area
    gdf["feature_length_m"] = gdf_projected.geometry.length
    ...
    gdf.to_file(
        OUT_GPKG,
        layer="oregon_osm_recognition_features_v04",
        driver="GPKG",
    )
