...
Adds local relief metrics to the UREM coastal grid.
...
Method:
- Uses cell centroids in EPSG:3310.
- For each cell, finds neighboring cells within 3 km.
- local_relief_m = max elevation in neighborhood - min elevation in neighborhood.
- relief_score = normalized local_relief_m from 0 to 1.
"""
...
INPUT_PATH = PROJECT_ROOT / "data/processed/coastal_grid_elevation.gpkg"
OUTPUT_PATH = PROJECT_ROOT / "data/processed/coastal_grid_relief.gpkg"
...
def setup_logger():
    logger = logging.getLogger("06_add_relief")
...
def main():
    logger = setup_logger()
    logger.info("Starting Script 06: Add local relief")
