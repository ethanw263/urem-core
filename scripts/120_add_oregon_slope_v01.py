#!/usr/bin/env python3
"""
120_add_oregon_slope_v01.py

Adds DEM-derived slope metrics to the Oregon Coast grid.
...
SCRIPT_NAME = "120_add_oregon_slope_v01"

PROJECT_ROOT = Path(__file__).resolve().parents[1]

INPUT_PATH = PROJECT_ROOT / "data/processed/oregon_coast_grid_relief_v01.gpkg"
OUTPUT_PATH = PROJECT_ROOT / "data/processed/oregon_coast_grid_slope_v01.gpkg"
...
def main():
    logger = setup_logger()
    logger.info("Starting Oregon DEM-derived slope computation")
    ...
    valid_slope = grid["slope_deg"].notna()

    logger.info("QA summary")
    logger.info(f"Cells processed: {len(grid):,}")
    logger.info(f"Cells with slope: {valid_slope.sum():,}")
    logger.info(f"Missing slope count: {(~valid_slope).sum():,}")
    logger.info(f"Unique tiles loaded this run: {len(tile_cache):,}")
    logger.info(f"Output CRS: {grid.crs}")

    if valid_slope.any():
        logger.info(f"Min slope: {grid.loc[valid_slope, 'slope_deg'].min():,.2f} deg")
        logger.info(f"Mean slope: {grid.loc[valid_slope, 'slope_deg'].mean():,.2f} deg")
        logger.info(f"Max slope: {grid.loc[valid_slope, 'slope_deg'].max():,.2f} deg")
    ...
    grid.to_file(
        OUTPUT_PATH,
        layer="oregon_coast_grid_slope_v01",
        driver="GPKG",
    )
