#!/usr/bin/env python3
"""
Script 37: Compute Recognition Score v04

Purpose:
- Convert expanded Recognition v04 features into final observed recognition score.
- Uses land-validity flag.
- Dampens extreme trail/parking counts.
"""
...
INPUT_PATH = BASE_DIR / "data/processed/recognition_features_v04.gpkg"

OUT_GPKG = BASE_DIR / "data/processed/recognition_score_v04.gpkg"
OUT_CSV = BASE_DIR / "data/processed/recognition_score_v04.csv"


def log(msg):
    print(f"[37_compute_recognition_score_v04] {msg}")
...
def main():
    ...
    gdf["observed_recognition_v04"] = safe_minmax(gdf["observed_recognition_v04_raw"])
    ...
    # v04 confidence is much better than v03 because trails/beaches/parks/protected areas exist.
    ...
    # Water cells should not be candidates later, but keep score for diagnostics.
    if "is_valid_land_candidate" in gdf.columns:
        gdf["recognition_valid_land_flag_v04"] = gdf["is_valid_land_candidate"].astype(bool)
    else:
